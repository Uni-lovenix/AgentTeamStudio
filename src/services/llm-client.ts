...
function buildUserPrompt(input: GenerateTeamInput): string {
  return `项目名称：${input.projectName || '未命名项目'}\n技术栈提示：${input.techStackHints || '未指定'}\n需求描述：${input.requirement}\n\n请设计一个适合这个项目的多智能体团队。\n规则：1. 从需求中识别完成项目必须承担的责任区块；2. 团队必须包含“规划者”和“评估者”，每个责任区块对应一个或多个开发者角色；3. 每个迭代开始前，规划者制定迭代协议，开发者按协议开发，评估者按协议校验并反馈给开发者修改；4. 为每个角色写具体职责，不要输出空职责；5. 不要只按前端、后端、数据库等实现功能拆分角色；6. 过程管理使用轻量 RUP 的启动、细化、构建、移交四阶段，不生成额外 RUP 角色；7. schemaVersion 必须是 3，每个角色必须带 kind；8. 输出 processManagement 和上述 RUP 工作流。`;
}
